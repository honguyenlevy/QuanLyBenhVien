...
ELSIF (COUNTT>0) THEN
UPDATE NHANVIEN SET HOTEN=TRIM(I_HOTEN),PHAI=TRIM(I_PHAI),NGAYSINH=TRIM(I_NGAYSINH), QUEQUAN=TRIM(I_QUEQUAN),SODT=TRIM(I_SODT),CSYT=TRIM(I_CSYT),VAITRO=TRIM(I_VAITRO),CHUYENKHOA=TRIM(I_CHUYENKHOA) WHERE TRIM(UPPER(MANV))=TRIM(UPPER(I_MANV));
